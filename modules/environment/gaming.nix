...
let
  ...
  cdda = (
    pkgsRepo.local.cdda.override rec {
      typeface = [
        "${pkgs.iosevka-comfy.comfy}/share/fonts/truetype/iosevka-comfy-normalboldupright.ttf"
      ];
            ...
            super.lutris.override (
              let
                ...
                mb_fmod = super.fmodex.overrideAttrs (prev: {
                  installPhase =
                    prev.installPhase
                    + ''ls $out/lib; ln -s $out/lib/libfmodex.so $out/lib/libfmodex64.so; ls $out/lib'';
                });
              in
              {
                extraPkgs = pkgs: [
