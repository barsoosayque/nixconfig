...
let
  ...
  mkEvents = eventDefs: foldl (acc: val: acc // val) { } (map mkEventOption eventDefs);

  mkRunScript =
    eventDef:
    let
      runCallbacks = cmd: concatStringsSep "\n" config.system.events."${eventDef.name}Callbacks"."${cmd}";
  ...
  mkRunScripts =
    ...
    listToAttrs (
      map (def: {
        name = "${def.name}Script";
        value = mkRunScript def;
      }) eventDefs
    );
in
let
  commands = [
    {
      name = "onStartup";
      description = "User system startup";
    }
    {
      name = "onReload";
      description = "User System reload";
    }
    {
      name = "onWMLoaded";
      description = "WL is loaded";
    }
    {
      name = "onTorrentDone";
      description = "Transmission torrent downloaded";
    }
    {
      name = "onScreenshot";
      description = "Screenshot made";
    }
  ];
in
{
  ...
  config = {
    helpers.mkAllEventsCallback =
      callbackName: script:
      listToAttrs (
        map (def: {
          name = "${def.name}Callbacks";
          value = {
            "${callbackName}" = [ script ];
          };
        }) commands
